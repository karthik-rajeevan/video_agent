...
from utils.export import build_report, export_pdf, export_txt
...
if st.session_state.result:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        pdf_path = export_pdf(report, os.path.join(tmp, "report.pdf"))
        txt_path = export_txt(report, os.path.join(tmp, "report.txt"))
    ...
    with d1:
        st.download_button(
            "📄 Download PDF", data=pdf_bytes, file_name=f"{r['title']}.pdf",
            mime="application/pdf", use_container_width=True,
        )
    with d2:
        st.download_button(
            "📃 Download TXT", data=txt_bytes, file_name=f"{r['title']}.txt",
